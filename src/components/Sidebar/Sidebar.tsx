    ...
    <div className="w-full md:w-1/3 flex flex-col gap-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="flex bg-[#539ba0] text-white">
          <button 
            onClick={() => { setActiveTab('display'); setSettings({...settings, isGroup: false}); }}
            className={cn(
              "flex-1 py-3 px-4 flex items-center justify-center gap-2 font-medium transition-colors",
              activeTab === 'display' ? "bg-white text-[#539ba0]" : "hover:bg-white/10"
            )}
          >
            <Layout size={18} /> Display Settings
          </button>
          <button 
            onClick={() => { setActiveTab('chat'); setSettings({...settings, isGroup: false}); }}
            className={cn(
              "flex-1 py-3 px-4 flex items-center justify-center gap-2 font-medium transition-colors border-x border-white/20",
              activeTab === 'chat' ? "bg-white text-[#539ba0]" : "hover:bg-white/10"
            )}
          >
            <Plus size={18} /> Add Chat
          </button>
          <button 
            onClick={() => { setActiveTab('group'); setSettings({...settings, isGroup: true}); }}
            className={cn(
              "flex-1 py-3 px-4 flex items-center justify-center gap-2 font-medium transition-colors",
              activeTab === 'group' ? "bg-white text-[#539ba0]" : "hover:bg-white/10"
            )}
          >
      ...
      <p className="text-xs text-gray-400 italic">
        👉 Facing any issues with current options or have new feature suggestions? Send us your feedback!
      </p>
    </div>
  );
};
