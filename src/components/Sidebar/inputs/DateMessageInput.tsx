...
export const DateMessageInput: React.FC<MessageInputProps> = ({
  msgText,
  setMsgText,
}) => {
  const [isCustomDate, setIsCustomDate] = React.useState(false);
  const [customDateVal, setCustomDateVal] = React.useState(
  ...
  React.useEffect(() => {
    if (isCustomDate && customDateVal) {
      ...
      if (!isNaN(dateObj.getTime())) {
        ...
        else if (customDateFormat === 1)
          formatted = `${dayName}, ${d} ${mShort}`;
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="flex flex-wrap gap-2">
        ...
        <button
          onClick={() => {
            setIsCustomDate(false);
            setMsgText("Yesterday");
          }}
          className={cn(
            "px-3 py-1.5 rounded-md text-sm border transition-all",
            !isCustomDate && msgText === "Yesterday"
              ? "bg-primary text-white border-primary"
              : "bg-white text-gray-600 hover:bg-gray-50",
          )}
        >
        ...
        <div className="space-y-4 pt-2 border-t border-gray-100">
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full p-2 rounded border border-gray-200 text-sm focus:ring-1 focus:ring-primary outline-none"
            />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-gray-400 uppercase">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                onClick={() => setCustomDateFormat(0)}
                className={cn(
                  "py-2 px-1 text-xs font-medium rounded border text-center transition-all",
                  customDateFormat === 0
      ...
      <div className="space-y-3 mt-4">
        ...
        <div className="relative border rounded-lg bg-chat-light p-4 flex flex-col gap-3 overflow-hidden text-sm">
          ...
          <div className="self-start max-w-[80%] bg-white px-2 py-1 flex items-end gap-2 rounded-lg shadow-sm">
            ...
            <span className="text-[9px] opacity-60 pb-0.5 whitespace-nowrap">
              04:25 pm
            </span>
          </div>
          ...
          <div className="self-end max-w-[80%] bg-bubble-user px-2 py-1 flex items-end gap-2 rounded-lg shadow-sm">
            <span>No..</span>
        ...
        <div className="flex items-start gap-2 pt-1 text-gray-500">
          <div className="w-4 h-4 rounded-full bg-gray-400 text-white flex items-center justify-center text-[10px] font-bold mt-0.5 shrink-0">
            i
          </div>
          <p className="text-xs">
            Add a date to separate messages, just like the example above.
          </p>
        </div>
      </div>
    </div>
