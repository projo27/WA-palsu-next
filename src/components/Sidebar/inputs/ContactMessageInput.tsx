  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b border-gray-200">
        <button
          ...
          className={cn(
            ...
            !isMultiple ? "bg-[#539ba0] text-white border-[#539ba0]" : "text-gray-500 hover:text-gray-700 bg-white"
          )}
        >
          Single Contact
        </button>
      ...
      <div className="space-y-2">
        <label className="text-xs font-semibold text-gray-500">Contact Detail</label>
        <div className="flex items-center gap-3">
          <div className="relative group cursor-pointer shrink-0" onClick={() => fileInputRef?.current?.click()}>
            {msgFile ? (
              <img src={msgFile} alt="Contact Avatar" className="w-12 h-12 rounded-full object-cover border border-gray-200" />
            ) : (
      ...
      {isMultiple && (
        <div className="space-y-2">
          <label className="text-xs font-semibold text-gray-500 flex items-center gap-1">
            Contact Count
            <div className="w-3.5 h-3.5 rounded-full border border-gray-400 text-gray-400 flex items-center justify-center text-[9px] font-bold">i</div>
          </label>
          <input
            type="number"
            min="1"
            ...
            className="w-full p-2 rounded border border-gray-200 text-sm focus:outline-none focus:border-[#539ba0]"
          />
        </div>
      )}
