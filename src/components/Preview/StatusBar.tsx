...
export const StatusBar: React.FC<StatusBarProps> = ({ settings }) => {
  ...
  return (
    <div className="px-6 py-2 flex justify-between items-center text-[10px] font-bold z-30">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-0.5">
          {settings.showBatteryPercentage && <span>{settings.batteryLevel}%</span>}
          <div className="w-5 h-2.5 border border-current rounded-sm relative p-0.5">
            <div 
              className={cn("h-full rounded-sm", settings.batteryLevel < 20 ? "bg-red-500" : "bg-current")} 
              style={{ width: `${settings.batteryLevel}%` }} 
            />
            {settings.isCharging && <BatteryCharging size={8} className="absolute inset-0 m-auto" />}
          </div>
        </div>
      </div>
    </div>
